...
typedef tree<long long, null_type, less_equal<long long>, rb_tree_tag, tree_order_statistics_node_update> multi_ordered_tree; 
    ...
    FOR(q) {
        ...
        if (s == "IN") {
            ...
            if (st.size() & 1) {
                ...
                ans = *st.find_by_order(mid-1) + *st.find_by_order(mid);
                ans /= 2.0;
            }
            double ans2 = floor(ans);
            if (abs(ans2 - ans) < 1e-6) {
