...
// 2 pragma lines give compiler information to use SIMD instruction for optimize
// code. #pragma GCC target("avx2") #pragma GCC optimize("O3")
...
bool inside(int x) { return 1 <= x && x <= layer[n]; }
...
void backtrack(int pos, int val) {
  ...
  if (*current_layer ==
      pos) { // End of row, we need to check if it is right with the top.

    int top = ans[pos - *prev(current_layer) - 1];
    if (abs(ans[pos] - ans[pos - 1]) != top)
      return; // wrong.
    else
      backtrack(pos + 1, val); // else continue to backtrack.
