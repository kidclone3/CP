// Test in CSES/Additional Problems/Maximum Xor Subarray.cpp
// Binary Trie
...
template <class Val = long long,        // Value store in trie
          class Count = long long,      // frequency of values
          int B = (sizeof(Val) * 8 - 2) // max number of bits
          >
BinaryTrie {
  struct Node {
  ...
  BinariesTrie() : nodes{Node()} {} // create root node

  // Number of elements in the trie
  Count size() { return nodes[0].count; }

  void insert(Val x, Count cnt = 1) { update(x, cnt); }
  void insert(Val x, Count cnt = -1) { update(x, cnt); }
  ...
  inline int get_bit(Val v, int bit) { return (v >> bit) & 1; }
};
