...
int dfs(vvi &a, vvb &check, int x, int y) {
    int res = 1;
    check[x][y] = true;
    FOR(8) {
        int xx = x+d8x[i];
        int yy = y+d8y[i];
        if (inside(xx, yy) && a[xx][yy] && !check[xx][yy]) {
            res += dfs(a, check, xx, yy);
            // cerr << res << '\n';
        }
    }
    // FOR(n) FOR(j, m) cout << check[i][j] << " \n"[j==m-1];
    return res;
}
int solve() { 
    cin >> n >> m;
    ...
    FOR(n) FOR(j, n) {
        if (a[i][j] && !check[i][j]) {
            int tmp = dfs(a, check, i, j);
            ans = max(ans, tmp);
            X ++;
        }
    }
    cout << X << " " << ans << "\n";
    FOR(n) FOR(j, m) cout << a[i][j] << " \n"[j==m-1];
    return 0; 
}
int main() {
  IOS;
